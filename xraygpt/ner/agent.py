...
def _gross_recognize_entities(text: str, llm: ChatOpenAI) -> List[str]:
    ...
    chat_template = ChatPromptTemplate(
        messages=[
            SystemMessagePromptTemplate.from_template("You are an agent to help me recognize named entities in text, give me the entities based on following rules\n1. Entities are only people's names\n2. Outputs should following the format: {format_instructions}"),
...
def _refine_recognized_entity(text: str, name: str, items: List[Item], llm) -> Tuple[List[str], Optional[Item]]:
    logger.debug("Refining recognized entity: {name} with {num_items}", name=name, num_items=len(items))
    response_schemas = [
            ResponseSchema(name="to_delete", description="An array of entity ids to delete", type="[int]"),
    ...
    reference_description = "\n".join([f"[{ix}]: {n}: {d}" for ix, (n,d) in enumerate(references)])
    ...
    if len(to_delete) != len(resp["to_delete"]):
        logger.warning("Some items to delete are invalid: {to_delete}", to_delete=resp["to_delete"])
    ...
    item_to_add = None
    if resp["entity_name"]:
        item_to_add = Item(id="", name=resp["entity_name"], description=resp["entity_description"])
    else:
